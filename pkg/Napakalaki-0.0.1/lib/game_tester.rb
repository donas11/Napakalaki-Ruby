    ...
    begin #Mientras dure la partida
      ...
      if (command == Command::SHOWMONSTER) then
        ...
        if (command == Command::COMBAT) then
          ...
          case combatResult
          when Napakalaki::WINANDWINGAME then 
            ...
            when Napakalaki::LOSEANDESCAPE then
              puts "\n\n Perdiste el combate pero has logrado escapar"
            when Napakalaki::LOSEANDDIE then
              puts "\n\n Perdiste el combate y ademas estas muerto"
            when Napakalaki::LOSEANDCONVERT then
              puts "\n\n Perdiste el combate y ademas te has convertido"
          end #case
          if (combatResult != Napakalaki::WINANDWINGAME) then
